...
class ShowContributorPageMenu(PageMenu):
    def __init__(self, queryset, request, slug, *args, **kwargs):
        self.items = [
            URLPatternItem(request, title="Blog", path=reverse('showcontributor_content_list', kwargs={'slug': slug}), matching_pattern_names=['showcontributor_content_list',], default=False),
            ...
            #URLPatternItem(request, title="Appearances", path=reverse('showcontributor_detail', kwargs={'slug': slug}), matching_pattern_names=['showcontributor_detail',], default=False),
        ]
